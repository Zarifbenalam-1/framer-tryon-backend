...
let currentHfToken = process.env.HF_TOKEN; // New: Runtime HF Token
...
const getProvider = () => {
    return ProviderFactory.getProvider(currentProviderName, {
        ...
        hfToken: currentHfToken, // Pass the token
        model: currentModel
    });
};
...
app.post('/api/admin/config', (req, res) => {
    const { apiKey, hfToken, model, provider } = req.body;
...
app.post('/api/admin/validate-key', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            currentProvider: currentProviderName,
            currentModel: currentModel,
            hasHfToken: !!currentHfToken, // Tell UI if token is set
